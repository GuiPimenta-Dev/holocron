...
def provider_from_env(env: dict[str, str]) -> EmbeddingProvider:
    ...
    if key := env.get("VOYAGE_API_KEY"):
        return VoyageEmbeddings(key)
    raise RuntimeError("no OPENAI_API_KEY or VOYAGE_API_KEY set — see .env.example")


def _post_with_retry(url: str, api_key: str, payload: dict[str, Any]) -> dict[str, Any]:
    # Free tiers rate-limit by requests-per-minute; a 30k-chunk run must
    # survive that instead of dying mid-way.
    for attempt in range(8):
        ...
        r.raise_for_status()
        return r.json()
    raise RuntimeError(f"still rate-limited after 8 retries: {url}")
